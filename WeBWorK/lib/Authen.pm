...
sub checkPassword($$$) {
	...
	# moodle uses md5 hashed passwords...
	my $possibleCryptPassword = md5_hex($possibleClearPassword);
...
sub checkKey($$$) {
	...
	if (time <= $Key->timestamp()+$ce->{sessionKeyTimeout}) {
		if ($possibleKey eq $Key->key()) {
			...
			$Key->timestamp(time);
			$db->putKey($Key);
			# also update Moodle session expire time
		...
		# expired, but is the moodle session expired?
		if( $self->moodleSessionExpired ) {
			# yup, kill this one as well.
			$db->deleteKey($userID);
			return 0;
		}
		else {
			# nope, this one will get extended later
			# but also be sure to extend the moodle session:
			$self->extendMoodleSession;
			return 1;
		}
	}
...
sub sendCookie {
	...
	my $cookie = Apache::Cookie->new($r,
		...
		-value   => "$userID\t$key",
		-expires => $expires,
		-domain  => $r->hostname, # for some reason, with this uncommented, the cookie isn't getting set -- may just be because I'm running the test env on port 81?
...
sub verify($) {
	...
	VERIFY: {
		...
		unless (defined $user or defined $passwd or defined $key) {
			...
			elsif( $moodleUser and $moodleExpires and time <= $moodleExpires ) {
				# There isn't a webwork cookie, but there _is_ a Moodle one, and it's valid
				# so we create a WeBWorK key for the user:
				my $Key = $self->generateKey($moodleUser);
				# something of a hack, I'm seeing weird issues with cookies not getting set?
				# anyway, if the key exists already, use put instead
				# this shouldn't be a security issue, as to get here, they already have a valid Moodle key.
				if( defined $db->getKey($moodleUser) ) {
					$db->putKey($Key);
				}
				else {
					$db->addKey($Key);
