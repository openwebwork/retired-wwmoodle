<?php // $Id: files.php,v 1.1 2007-09-11 22:53:00 mleventi Exp $
...
//  USER is a persistent variable using sessions
    ...
    require($CFG->dirroot.'/question/type/webwork/locallib.php');
    ...
    if($qid == -1) {
        $searcher = webwork_get_tmp_path();
    } else {
        $searcher = webwork_get_wwquestion_path($qid);
    }
    ...
    switch ($action) {
        ...
        case "upload":
            ...
            if ($save and confirm_sesskey()) {
                ...
                echo "<form enctype=\"multipart/form-data\" method=\"post\" action=\"files.php\">";
                echo "<div>";             
            ...
            if ($confirm and confirm_sesskey()) {
                ...
                if (setfilelist($_POST)) {
                    ...
                    if (empty($CFG->resource_blockdeletingfile) or $block == '') {
                        $optionsyes = array('id'=>$id, 'qid'=>$qid, 'wdir'=>$wdir, 'action'=>'delete', 'confirm'=>1, 'sesskey'=>sesskey(), 'choose'=>$choose);
                        $optionsno  = array('id'=>$id, 'qid'=>$qid, 'wdir'=>$wdir, 'action'=>'cancel', 'choose'=>$choose);
                        notice_yesno (get_string('deletecheckfiles'), 'files.php', 'files.php', $optionsyes, $optionsno, 'post', 'get');
                    } else {

                        notify(get_string('warningblockingdelete', 'resource'));
                        $options  = array('id'=>$id, 'qid'=>$qid, 'wdir'=>$wdir, 'action'=>'cancel', 'choose'=>$choose);
        ...
        case "makedir":
            if (($name != '') and confirm_sesskey()) {
                ...
                } else if (! make_upload_directory("$wdir/$name")) {
        ...
        case "unzip":
            ...
            if (($file != '') and confirm_sesskey()) {
                ...
                echo "<div style=\"text-align:center\"><form action=\"files.php\" method=\"get\">";
                echo "<div>";
                echo ' <input type="hidden" name="choose" value="'.$choose.'" />';
                echo " <input type=\"hidden\" name=\"id\" value=\"$id\" />";
                echo " <input type=\"hidden\" name=\"qid\" value=\"$qid\" />";
        ...
        case "listzip":
            ...
            if (($file != '') and confirm_sesskey()) {
                ...
                echo "<br /><center><form action=\"files.php\" method=\"get\">";
                echo "<div>";
                echo ' <input type="hidden" name="choose" value="'.$choose.'" />';
                echo " <input type=\"hidden\" name=\"id\" value=\"$id\" />";
                echo " <input type=\"hidden\" name=\"qid\" value=\"$qid\" />";
        ...
        case "restore":
            ...
            if (($file != '') and confirm_sesskey()) {
                ...
                notice_yesno (get_string("areyousuretorestorethis"),
                                $restore_path."?id=".$id."&amp;qid=".$qid."&amp;file=".cleardoubleslashes($id.$wdir."/".$file)."&amp;method=manual",
                                "files.php?id=$id&amp;qid=$qid&amp;wdir=$wdir&amp;action=cancel");
            } else {
                displaydir($wdir);
            }
            //html_footer();
...
//  $wdir == / or /a or /a/b/c/d  etc
    ...
    echo "</td>";
    echo "<td align=\"right\">";
        echo "<form action=\"files.php\" method=\"get\">"; //dummy form - alignment only
        echo "<fieldset class=\"invisiblefieldset\">";
